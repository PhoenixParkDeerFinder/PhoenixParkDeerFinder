      ...
      <div style={styles.modal} onClick={e => e.stopPropagation()}>
        ...
        {step === 'form' && (
          <>
            ...
            <form onSubmit={handleSubmit}>
              ...
              {animalsLoading
                ? <div style={styles.muted}>Loading animals…</div>
                : (
                  <select
                    style={styles.select}
                    value={animalId ?? ''}
        ...
        {step === 'success' && (
          <div style={styles.centered}>
            ...
            <div style={{ fontWeight: 600, marginBottom: 6 }}>Sighting logged!</div>
            <div style={styles.muted}>Your pin will appear on the map shortly.</div>
            <button style={{ ...styles.btnPrimary, marginTop: 20 }} onClick={onClose}>
              Close
            </button>
          </div>
        )}
        ...
        {step === 'blocked' && (
          <div style={styles.centered}>
            ...
            <div style={{ fontWeight: 600, marginBottom: 6 }}>Too many sightings</div>
            <div style={styles.muted}>
              You've reached the limit for anonymous submissions. Please try again later.
            </div>
            <button style={{ ...styles.btnSecondary, marginTop: 20 }} onClick={onClose}>
              Close
            </button>
          </div>
        )}
...
function ModalHeader({ title, onClose }: { title: string; onClose: () => void }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
      ...
      <button onClick={onClose} style={styles.btnClose} aria-label="Close">✕</button>
    </div>
  )
}
...
const styles: Record<string, React.CSSProperties> = {
  ...
  btnClose: {
    ...
    cursor: 'pointer', fontSize: 16, color: '#6b6a65', padding: 4,
  },
